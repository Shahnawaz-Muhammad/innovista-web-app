...
router.get("/CategoryCount", async (req, res) => {
  try {
    ...
    const freelancerCount = await User.countDocuments({
      category: "Freelancer",
    });
...
router.get("/StationCategoryCount/:bookingStation", async (req, res) => {
  const { bookingStation } = req.params;

  try {
    // Count the total number of documents in the "signup" collection
    const totalRecords = await User.countDocuments({bookingStation: bookingStation});
    ...
    // Count the number of groups
    const groupCount = await User.countDocuments({
      category: "Group",
      bookingStation: bookingStation,
    });
